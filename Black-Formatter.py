...
class Settings:
	...
	def verify_settings(cls) -> None:
		...
		for k, v in cls.data["config"].items():
			if type(v) == str and "python_" in k and "path" in k and PLATFORM.lower() in k:
				python_path_exist = True if os_path.exists(v) else (k, v)

			if type(v) == str and "black_" in k and "path" in k and PLATFORM.lower() in k:
				if "local" in k:
...
class FormatBlackCommand(sublime_plugin.TextCommand):
	def run(self, edit) -> None:
		...
		cmd_black = [
			settings["config"][f"local_black_path.{PLATFORM}".lower()] or settings["config"][f"black_path.{PLATFORM}".lower()]
		]
